...
public:
	...
	virtual cVector3iArray Update(cWorld & a_World, Vector3i a_Position, BLOCKTYPE a_BlockType, NIBBLETYPE a_Meta, PoweringData a_PoweringData) const override
	{
		...
		// Determine if the repeater is locked by another
		// Ignore and forget all power change requests if so
...
private:
	...
	static bool DoesLhsLockMe(NIBBLETYPE a_MetaLhs, NIBBLETYPE a_MyMeta)
	{
		...
		// Convenience function to compare metadata
		const auto CheckMetas = [&](NIBBLETYPE a_ExpectedLhs, NIBBLETYPE a_MyExpected)
		{
			return (a_MetaLhs == a_ExpectedLhs) && (a_MyMeta == a_MyExpected);
		};
		...
		*/
		return
			CheckMetas(0x01, 0x00) ||
			CheckMetas(0x02, 0x01) ||
			CheckMetas(0x03, 0x02) ||
			CheckMetas(0x00, 0x03)
		;
	}
};
